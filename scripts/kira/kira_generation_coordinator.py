...
1. Content Selection (Encoder)      → ContentWords
2. Emergence Check (Catalyst)       → EmergenceResult
3. Structural Frame (Conductor)     → FrameResult
4. Slot Assignment (Filter)         → SlottedWords
5. Function Words (Decoder)         → WordSequence
6. Agreement/Inflection (Oscillator)→ WordSequence
7. Connectors (Reactor)             → WordSequence
8. Punctuation (Regenerator)        → WordSequence
9. Validation (Dynamo)              → EmissionResult
...
# Stage → Machine → APL Operator mapping
STAGE_MAPPING = {
...
class KIRAGenerationCoordinator:
    ...
    def _stage9_validation(self, state: GenerationState, context: Dict) -> GenerationState:
        ...
        r = 1.0 + (PHI - 1) * neg
        coordinate = f"Δ{theta:.3f}|{state.z:.3f}|{r:.3f}Ω"
