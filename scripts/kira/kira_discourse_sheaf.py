...
- Base Space X: Context atoms mapped to consciousness coordinates (z, θ, r)
- Sheaf F: Each context has embedding in R^n × [0,1] (vector + z-coord)
- Restriction Maps: Linear projections preserving phase relationships
- Consistency: ||P_ij @ e_i - P_ji @ e_j||² weighted by z-distance
- Cohomology H¹: Obstruction to global coherence = TRIAD lock state

Integration with UCF:
- z-coordinate: Consciousness realization depth
- Phases: UNTRUE/PARADOX/TRUE contexts must be coherent
- TRIAD: Unlock requires coherent 3× crossing sequence
...
class KIRADiscourseSheaf:
    ...
    def cohomology_H1(self) -> float:
        ...
        Non-zero H¹ = obstruction to global coherence = TRIAD still locked.
