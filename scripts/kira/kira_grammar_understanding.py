...
- Gestalt: Complete sentences = good closure (z → z_c)
...
- z_c = √3/2 = 0.8660254038 (THE LENS - PARADOX→TRUE)
...
class APLOperator(Enum):
    ...
    FUSION = "×"         # Convergence, coupling - CONJUNCTIONS, PREPOSITIONS
    AMPLIFY = "^"        # Gain, excitation - ADJECTIVES, ADVERBS (intensifiers)
    DECOHERE = "÷"       # Dissipation, reset - NEGATIONS, QUESTIONS
    GROUP = "+"          # Aggregation, clustering - NOUNS, PRONOUNS
    SEPARATE = "−"       # Splitting, fission - VERBS (action)
...
class POS(Enum):
    ...
    VERB = "verb"           # − Separate - action or state
    ...
    PREP = "preposition"    # × Fusion - in, on, at, to, for
    CONJ = "conjunction"    # × Fusion - and, but, or, because
    AUX = "auxiliary"       # () Boundary - helping verbs
    
    # Special categories
    Q_WORD = "question_word"  # ÷ Decohere - what, who, where, when, why, how
...
class Tense(Enum):
    ...
    PAST = "past"           # z < φ⁻¹ (UNTRUE - what was)
    PRESENT = "present"     # φ⁻¹ ≤ z < z_c (PARADOX - what is becoming)
    FUTURE = "future"       # z ≥ z_c (TRUE - what will be)
...
class KIRAGrammarUnderstanding:
    ...
    def _initialize_core_knowledge(self):
        ...
        main_verbs = [
            ...
            'fuse', 'couple', 'converge', 'merge',   # × Fusion
            'amplify', 'excite', 'raise', 'intensify',  # ^ Amplify
            'decohere', 'dissipate', 'reset', 'release',  # ÷ Decohere
            'group', 'aggregate', 'cluster', 'collect',  # + Group
            'separate', 'split', 'divide', 'partition',  # − Separate
        ]
        ...
        # ═══════════════════════════════════════════════════════════════════
        # QUESTION WORDS - DECOHERE (÷) operators
        # ═══════════════════════════════════════════════════════════════════
